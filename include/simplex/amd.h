    ...
    CSR strict_upper_union_transpose() const {
        ...
        if (wr.back() != (i32)U.indices.size()) {
            ...
            for (i32 i = 0; i < N; ++i) {
                ...
                std::copy(U.indices.begin() + rb, U.indices.begin() + re, nidx.begin() + outb);
            }
            U.indptr.swap(nip);
            U.indices.swap(nidx);
        }
...
class AMDReorderingArray {
  ...
  private:
    ...
    void apply_dense_postponement_() {
        ...
        const i32 dense_cut =
            (dense_cutoff_ > 0)
                ? dense_cutoff_
                : std::max<i32>(
                      16, std::min(n - 1, (i32)std::floor(0.35 * avg +
    ...
    void grow_iw_(i32 need) {
        i32 new_cap =
            std::max(need, (i32)(std::max(2 * (i64)nzmax_, (i64)nzmax_ + nzmax_ / 2) + 1024));
        iw_.resize(new_cap);
        nzmax_ = new_cap;
    ...
    void merge_variable_into_rep_(i32 rep, i32 u, const std::vector<i32>& rep_elem_list) {
        ...
        if (!sv_members_[u].empty()) {
            sv_members_[rep].insert(sv_members_[rep].end(), sv_members_[u].begin(),
                                    sv_members_[u].end());
            sv_members_[u].clear();
        }
    ...
    void eliminate_pivot_build_element_(i32 piv) {
        ...
        auto try_push = [&](i32 v) {
            if (0 <= v && v < n_ && elen_[v] == -1 && var_active_[v] && nv_[v] > 0 &&
                w_[v] != tag) {
                w_[v] = tag;
...
int main() {
    ...
    auto [perm, st] = amd.compute_fill_reducing_permutation(A, /*symmetrize=*/true);

    std::cout << "n=" << st.matrix_size << " nnz=" << st.original_nnz
              << " bw0=" << st.original_bandwidth << " bw1=" << st.reordered_bandwidth
              << " red=" << (100.0 * st.bandwidth_reduction) << "%\n";
    std::cout << "absorbed=" << st.absorbed_elements << " coalesced=" << st.coalesced_variables
